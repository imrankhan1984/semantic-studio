...
2. **Can you see focus?** `index.css` carries one global `:focus-visible` rule
   ...
   the home screen, and it is not a precedent.
...
with a sensible accessible name. **A name is not the only thing it can be wrong
about.** On 2026-08-04 the home screen shipped a tablist in which two tabs
reported `aria-selected="true"` at once — Home, and whichever mode had been
pressed while nothing was open. Every control had a name, nothing looked wrong
on screen because the styling agreed with the sensible reading, and no test
asked. Check state as well as naming: selected, pressed, expanded, checked. Then tab through the interface and confirm you
