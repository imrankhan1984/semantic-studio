...
```bash
cd backend  && python -m pytest tests    # 48 tests
cd frontend && npm run test              # 61 tests, vitest
```

Both suites must pass before any change is considered done.

**Know the gap.** All 61 frontend tests are in `src/sparql/`. **No React
...
Project skills live in `.Codex/skills/` and load automatically.
...
Run `/run-skill-generator` once if `/run` and `/verify` do not know how to launch
this app.

## Known state, so you do not rediscover it

- **Three open security defects**, all demonstrated against a running instance:
  URL fetching reaches internal addresses, a SPARQL `SERVICE` clause makes the
  server call out, and uploads have no size limit or parse timeout. A
  specification exists. Do not work around them; fix them properly when asked.
